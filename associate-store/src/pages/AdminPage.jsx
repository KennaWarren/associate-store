...
function ImageUploader({ value, onChange, label="Upload Image", small=false }) {
  ...
  const handleFile = async (e) => {
    ...
    setCompressing(true);
    try { const c = await compressImage(file); onChange(c); }
  ...
  return (
    <div>
      ...
      {compressing ? (
        <div style={{ width:small?72:"100%", height:small?72:120, border:"2px dashed #EAEAEA", borderRadius:small?10:12, background:"#F7F7F7", display:"flex", alignItems:"center", justifyContent:"center" }}>
        ...
        <button onClick={() => ref.current.click()} style={{ width:small?72:"100%", height:small?72:120, border:"2px dashed #EAEAEA", borderRadius:small?10:12, background:"#F7F7F7", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:6, transition:"border-color 0.15s" }}
    ...
    <div style={{ background:"#F7F7F7", minHeight:"100vh" }}>
      <div style={{ background:"#fff", borderBottom:"1px solid #EAEAEA", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
        <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 32px", display:"flex", gap:4, alignItems:"center" }}>
          <span style={{ fontSize:13, fontWeight:700, color:"#1a1a1a", marginRight:16 }}>Admin</span>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{
              background: tab===t ? "#FFF0F0" : "none", border:"none",
              color: tab===t ? "#A22325" : "#888",
              padding:"16px 18px", fontSize:13, fontWeight: tab===t ? 700 : 500,
...
function OrdersTab() {
  ...
  const printPackingSlip = (order, payMethod) => {
    const itemRows = order.items.map(item => {
      ...
      return `<tr>
        <td style="padding:10px 8px;border-bottom:1px solid #eee;font-size:14px;">${item.productName}${variants?` <span style="color:#888;font-size:12px;">(${variants})</span>`:""}</td>
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Packing Slip — ${order.id}</title>
    <style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Helvetica Neue',sans-serif;color:#1a1a1a;padding:40px;max-width:680px;margin:0 auto}@media print{body{padding:20px}.no-print{display:none}}</style></head><body>
    <div style="display:flex;justify-content:space-between;margin-bottom:32px;padding-bottom:20px;border-bottom:3px solid #A22325;">
      <div><h1 style="font-family:Georgia,serif;font-size:28px;margin-bottom:4px;">Packing Slip</h1><p style="font-size:12px;color:#888;text-transform:uppercase;letter-spacing:.08em;">Associate Store</p></div>
      <div style="text-align:right;"><p style="font-size:13px;font-family:monospace;color:#A22325;font-weight:700;margin-bottom:4px;">${order.id}</p><p style="font-size:12px;color:#888;">${new Date(order.date).toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"})}</p></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:32px;">
      <div style="background:#F7F7F7;border-radius:10px;padding:16px 20px;"><p style="font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#bbb;margin-bottom:10px;">Ship To</p>
        <p style="font-size:15px;font-weight:700;margin-bottom:4px;">${order.name}</p><p style="font-size:13px;color:#555;margin-bottom:2px;">${order.email}</p><p style="font-size:13px;color:#555;">Store #: ${order.department}</p></div>
      <div style="background:#F7F7F7;border-radius:10px;padding:16px 20px;"><p style="font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#bbb;margin-bottom:10px;">Payment</p>
        <p style="font-size:13px;color:#555;margin-bottom:2px;">${payMethod?.label||order.paymentMethod}</p><p style="font-size:12px;color:#bbb;">Status: ${order.paid?"✓ Paid":"Pending"}</p></div>
    </div>
    ...
    <div style="display:flex;justify-content:flex-end;"><div style="width:240px;">
      ${order.discount>0?`<div style="display:flex;justify-content:space-between;font-size:13px;color:#555;margin-bottom:6px;"><span>Discount (${order.couponCode})</span><span style="color:#166534;">−$${(order.discount||0).toFixed(2)}</span></div>`:""}
      <div style="display:flex;justify-content:space-between;font-size:17px;font-weight:800;padding-top:10px;border-top:2px solid #1a1a1a;"><span>Total</span><span style="color:#A22325;">$${order.total.toFixed(2)}</span></div>
    </div></div>
    ${order.notes?`<div style="margin-top:24px;padding:14px 18px;background:#FFFBF5;border:1px solid #F5E0C0;border-radius:10px;"><p style="font-size:11px;font-weight:700;text-transform:uppercase;color:#A22325;margin-bottom:6px;">Notes</p><p style="font-size:13px;color:#555;">${order.notes}</p></div>`:""}
    <div style="margin-top:40px;padding-top:20px;border-top:1px solid #EAEAEA;text-align:center;"><p style="font-size:11px;color:#ccc;">Thank you — Associate Store · Internal Use Only</p></div>
    <div class="no-print" style="margin-top:32px;text-align:center;"><button onclick="window.print()" style="background:#A22325;color:#fff;border:none;border-radius:10px;padding:12px 32px;font-size:14px;font-weight:700;cursor:pointer;">Print</button></div>
    </body></html>`;
  ...
  const exportCSV = () => {
    const rows = [
      ["Order ID","Date","Name","Email","Store Number","Items","Subtotal","Discount","Coupon","Total","Payment","Paid","Status","Notes"],
      ...orders.map(o => [
    ...
    <div style={{ maxWidth:1100, margin:"0 auto" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28, flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={loadOrders} style={actionBtn("#666")}>↻ Refresh</button>
...
function ProductsTab() {
  ...
  const openEdit = (p) => { setForm({...p, price:String(p.price), cost:String(p.cost||""), image:p.image||"", variants:p.variants||{}, variantImages:p.variantImages||{}}); setEditing(p.id); setVariantKey(""); setVariantVals(""); };
  ...
  return (
    <div style={{ maxWidth:1100, margin:"0 auto" }}>
      ...
      {productsError && <div style={{ background:"#FEF2F2", border:"1px solid #FECACA", borderRadius:10, padding:"12px 16px", marginBottom:20, fontSize:13, color:"#991B1B" }}>Could not load products: {productsError}</div>}
        ...
        {products.map(p => (
          <div key={p.id} style={{ background:"#fff", border:"1px solid #EAEAEA", borderRadius:16, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,0.04)" }}>
            <div style={{ background:"#F7F7F7", height:130, display:"flex", alignItems:"center", justifyContent:"center", fontSize:44, overflow:"hidden" }}>
              {p.image ? <img src={p.image} alt={p.name} style={{width:"100%",height:"100%",objectFit:"contain",padding:"4px"}}/> : "📦"}
            </div>
            <div style={{ padding:"14px 16px" }}>
              ...
              {Object.keys(p.variants||{}).length>0 && <p style={{ fontSize:11, color:"#ddd", marginBottom:12 }}>Variants: {Object.keys(p.variants).join(", ")}</p>}
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>openEdit(p)} style={{...smBtn("#1a1a1a"),flex:1}}>Edit</button>
                {confirmDel===p.id ? (
                  <><button onClick={()=>{deleteProduct(p.id);setConfirmDel(null);}} style={smBtn("#A22325")}>Confirm</button><button onClick={()=>setConfirmDel(null)} style={smBtn("#888")}>Cancel</button></>
...
function CouponsTab() {
  ...
  return (
    <div style={{ maxWidth:800, margin:"0 auto" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:28 }}>
        <div><h2 style={{ fontFamily:"'Georgia', serif", fontSize:26, color:"#1a1a1a", marginBottom:4 }}>Coupons</h2><p style={{ color:"#bbb", fontSize:13 }}>{coupons.length} coupon{coupons.length!==1?"s":""}</p></div>
          ...
          {coupons.map(c => (
            <div key={c.id} style={{ background:"#fff", border:"1px solid #EAEAEA", borderRadius:14, padding:"18px 22px", display:"flex", alignItems:"center", gap:16, boxShadow:"0 2px 8px rgba(0,0,0,0.04)", borderLeft:`4px solid ${c.active?"#16a34a":"#EAEAEA"}` }}>
              ...
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                ...
                {confirmDel===c.id ? (
                  <><button onClick={()=>{deleteCoupon(c.id);setConfirmDel(null);}} style={smBtn("#A22325")}>Confirm</button><button onClick={()=>setConfirmDel(null)} style={smBtn("#888")}>Cancel</button></>
...
function SuggestionsTab() {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:28, flexWrap:"wrap", gap:12 }}>
        <div>
          <h2 style={{ fontFamily:"'Georgia', serif", fontSize:26, color:"#1a1a1a", marginBottom:4 }}>Product Suggestions</h2>
          <p style={{ color:"#bbb", fontSize:13 }}>{suggestionsLoading?"Loading…":`${suggestions.length} total · ${suggestions.filter(s=>!s.reviewed).length} unreviewed`}</p>
        </div>
      ...
      : (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {filtered.map(s => (
            <div key={s.id} style={{ background:"#fff", border:"1px solid #EAEAEA", borderRadius:14, padding:"20px 22px", boxShadow:"0 2px 8px rgba(0,0,0,0.04)", borderLeft:`4px solid ${s.reviewed?"#16a34a":"#A22325"}` }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12, flexWrap:"wrap", gap:8 }}>
                <div>
                  <p style={{ fontWeight:700, fontSize:15, color:"#1a1a1a", marginBottom:2 }}>{s.name}</p>
              ...
              <div style={{ background:"#F7F7F7", borderRadius:10, padding:"14px 16px", marginBottom:s.question?12:16 }}>
                <p style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:"#bbb", marginBottom:8 }}>Product Suggestion</p>
                <p style={{ fontSize:14, color:"#333", lineHeight:1.6 }}>{s.suggestion}</p>
              </div>
              {s.question && (
                <div style={{ background:"#FFFBF5", border:"1px solid #F5E0C0", borderRadius:10, padding:"14px 16px", marginBottom:16 }}>
                  <p style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:"#A22325", marginBottom:8 }}>Question / Comment</p>
                  <p style={{ fontSize:14, color:"#555", lineHeight:1.6 }}>{s.question}</p>
                </div>
              )}
              <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                ...
                {confirmDel===s.id ? (
                  <div style={{display:"flex",gap:10,alignItems:"center"}}>
                    <span style={{fontSize:13,color:"#A22325",fontWeight:500}}>Delete?</span>
...
function EmptyState({ icon, text }) {
  return <div style={{ textAlign:"center", padding:"80px 0" }}><p style={{ fontSize:36 }}>{icon}</p><p style={{ fontSize:15, marginTop:12, color:"#bbb" }}>{text}</p></div>;
}
function PField({ label, children }) {
  return <div><label style={{ display:"block", fontSize:11, fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", color:"#aaa", marginBottom:8 }}>{label}</label>{children}</div>;
}
